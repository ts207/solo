...
base_dir = '/home/irene/Edge/analysis/context_uplift_expanded'
combinations = []
for year in ["2023", "2024", "2025"]:
    for slice_label in [f"full_{year}", f"h1_{year}", f"h2_{year}"]:
        for symbol in ["BTCUSDT", "ETHUSDT"]:
            combinations.append((symbol, slice_label))
...
matrix.to_parquet(os.path.join(base_dir, 'matrix_expanded.parquet'), index=False)
matrix.to_csv(os.path.join(base_dir, 'matrix_expanded.csv'), index=False)
print("Saved matrix_expanded.parquet and matrix_expanded.csv")
...
if all_rejections:
    ...
    rejections_df.to_csv(os.path.join(base_dir, 'matrix_expanded_rejections.csv'), index=False)
    print("Saved matrix_expanded_rejections.csv")
