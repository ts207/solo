...
def test_deploy_boundary_protection(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "project/cli.py", "deploy", "paper", "--run_id", "raw_run", "--data_root", str(data_root)],
        capture_output=True,
        text=True
    )
...
def test_public_terminology_help():
    ...
    forbidden = ["trigger", "proposal", "certification"] # 'strategy' is more nuanced but 'thesis' preferred
...
def test_readme_model_consistency():
    ...
    assert "Strategy" not in readme # README should be fully clean
