...
def _detect_liquidation_cascade(*, symbol: str, timeframe: str, move_bps: float | None, market_features: Mapping[str, Any], config: Mapping[str, Any]) -> DetectedEvent | None:
    signed_move = float(move_bps or 0.0)
    oi_delta_fraction = float(market_features.get('open_interest_delta_fraction', market_features.get('oi_delta_fraction', 0.0)) or 0.0)
    ...
    detector_strength = abs(signed_move) / max(min_abs_move_bps, 1.0) + abs(oi_delta_fraction) / max(min_abs_oi_drop_fraction, 1e-9) + abs(funding_rate) / max(min_abs_funding_rate, 1e-9)
    return _build_detected_event(
        ...
        event_confidence=min(1.0, 0.7 + abs(oi_delta_fraction) / max(min_abs_oi_drop_fraction * 4.0, 1e-9)),
...
def detect_live_event(
    *,
    ...
    supported = [str(item).strip().upper() for item in list(configured_supported or ['VOL_SHOCK']) if str(item).strip()]
    if not supported:
        supported = ['VOL_SHOCK']
