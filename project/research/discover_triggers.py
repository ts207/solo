...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, required=True, choices=["parameter_sweep", "feature_cluster"]
    )
    parser.add_argument(
        "--family", type=str, default="vol_shock", help="Detector family for parameter sweep"
    )
    parser.add_argument("--symbol", type=str, default="BTCUSDT")
    parser.add_argument("--timeframe", type=str, default="5m")
    parser.add_argument("--data_root", type=str, default=str(PROJECT_ROOT.parent / "data"))
    parser.add_argument(
        "--out_dir", type=str, default=str(PROJECT_ROOT.parent / "data" / "trigger_proposals")
    )
    ...
    try:
        ...
        log.warning(
            f"Could not fully load canonical features: {e}. Falling back to empty structure."
        )
    ...
    if args.mode == "parameter_sweep":
        ...
        grid = {"z_threshold": [1.5, 2.0, 2.5, 3.0], "lookback_bars": [48, 96, 288]}
        proposals, trigger_cols = generate_parameter_sweep(
            features, family_grid={args.family: grid}
        )
    ...
    log.info(
        "Scoring proposals using canonical evidence pipeline (Fold Stability / Significance)..."
    )
