...
def _run_deploy_list_theses(args: argparse.Namespace) -> int:
    ...
    runs = sorted(path.name for path in thesis_root.iterdir() if path.is_dir()) if thesis_root.exists() else []
...
def _run_deploy_bind_config(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "freshness_streams": [{"symbol": symbol.strip().lower()} for symbol in args.symbols.split(",") if symbol.strip()],
    }
