...
def build_governance_artifacts(output_dir: Path) -> dict[str, object]:
    ...
    runtime_rows = [[r["event_name"], r["event_version"], r["role"], r["maturity"], r["runtime_default"], r["promotion_eligible"], r["supports_confidence"], r["emits_quality_flag"]] for r in inventory_rows]
    (output_dir / 'detector_runtime_matrix.md').write_text('# Detector Runtime Matrix\n\n' + _md_table(['Event','Version','Role','Maturity','Runtime Eligible','Promotion Eligible','Confidence','Quality Flag'], runtime_rows) + '\n', encoding='utf-8')

    promotion_rows = [[r["event_name"], r["event_version"], r["maturity"], r["primary_anchor_eligible"], r["runtime_default"], r["promotion_eligible"]] for r in inventory_rows]
    (output_dir / 'detector_promotion_matrix.md').write_text('# Detector Promotion Matrix\n\n' + _md_table(['Event','Version','Maturity','Primary Anchor','Runtime Eligible','Promotion Eligible'], promotion_rows) + '\n', encoding='utf-8')
    ...
    for c in contracts:
        ...
        calibration_rows.append([c.event_name, c.event_version, c.threshold_schema_version, c.calibration_mode, symbol_group, timeframe_group])
    (output_dir / 'detector_calibration_matrix.md').write_text('# Detector Calibration Matrix\n\n' + _md_table(['Event','Version','Threshold Version','Calibration Mode','Symbol Group','Timeframe Group'], calibration_rows) + '\n', encoding='utf-8')
    ...
    for role in sorted(grouped):
        ...
        for row in rows:
            role_parts.append(f"- `{row['event_name']}` — version `{row['event_version']}`, maturity `{row['maturity']}`, runtime `{row['runtime_default']}`, promotion `{row['promotion_eligible']}`")
        role_parts.append('')
