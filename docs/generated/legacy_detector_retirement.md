# Legacy Detector Retirement

Legacy detectors: **40**
Legacy detectors retired-safe: **40**

| Event | Role | Runtime Eligible | Promotion Eligible | Primary Anchor | Retired Safe |
|---|---|---|---|---|---|
| ABSORPTION_PROXY | trigger | False | False | False | True |
| BAND_BREAK | trigger | False | False | False | True |
| CHOP_TO_TREND_SHIFT | trigger | False | False | False | True |
| CLIMAX_VOLUME_BAR | trigger | False | False | False | True |
| COPULA_PAIRS_TRADING | research_only | False | False | False | True |
| DELEVERAGING_WAVE | trigger | False | False | False | True |
| DEPTH_STRESS_PROXY | trigger | False | False | False | True |
| FAILED_CONTINUATION | trigger | False | False | False | True |
| FALSE_BREAKOUT | trigger | False | False | False | True |
| FEE_REGIME_CHANGE_EVENT | trigger | False | False | False | True |
| FLOW_EXHAUSTION_PROXY | trigger | False | False | False | True |
| FORCED_FLOW_EXHAUSTION | trigger | False | False | False | True |
| FUNDING_TIMESTAMP_EVENT | context | False | False | False | True |
| GAP_OVERSHOOT | trigger | False | False | False | True |
| LIQUIDATION_EXHAUSTION_REVERSAL | trigger | False | False | False | True |
| MOMENTUM_DIVERGENCE_TRIGGER | trigger | False | False | False | True |
| ORDERFLOW_IMBALANCE_SHOCK | trigger | False | False | False | True |
| OVERSHOOT_AFTER_SHOCK | trigger | False | False | False | True |
| POST_DELEVERAGING_REBOUND | trigger | False | False | False | True |
| PRICE_VOL_IMBALANCE_PROXY | trigger | False | False | False | True |
| PULLBACK_PIVOT | trigger | False | False | False | True |
| RANGE_BREAKOUT | trigger | False | False | False | True |
| SCHEDULED_NEWS_WINDOW_EVENT | context | False | False | False | True |
| SEQ_FND_EXTREME_THEN_BREAKOUT | composite | False | False | False | True |
| SEQ_LIQ_VACUUM_THEN_DEPTH_RECOVERY | composite | False | False | False | True |
| SEQ_OI_SPIKEPOS_THEN_VOL_SPIKE | composite | False | False | False | True |
| SEQ_VOL_COMP_THEN_BREAKOUT | composite | False | False | False | True |
| SESSION_CLOSE_EVENT | context | False | False | False | True |
| SESSION_OPEN_EVENT | context | False | False | False | True |
| SLIPPAGE_SPIKE_EVENT | trigger | False | False | False | True |
| SPREAD_BLOWOUT | trigger | False | False | False | True |
| SPREAD_REGIME_WIDENING_EVENT | trigger | False | False | False | True |
| SUPPORT_RESISTANCE_BREAK | trigger | False | False | False | True |
| SWEEP_STOPRUN | trigger | False | False | False | True |
| TREND_ACCELERATION | trigger | False | False | False | True |
| TREND_DECELERATION | trigger | False | False | False | True |
| TREND_EXHAUSTION_TRIGGER | trigger | False | False | False | True |
| TREND_TO_CHOP_SHIFT | trigger | False | False | False | True |
| WICK_REVERSAL_PROXY | trigger | False | False | False | True |
| ZSCORE_STRETCH | trigger | False | False | False | True |
