...
class CampaignController:
    ...
    def _propose_next_request(self) -> Optional[Dict[str, Any]]:
        ...
        Priority order:
          ...
          2. Exploit — consume explicit exploit queue, then reflection/promising exploit.
          3. Retest  — consume explicit retest queue for structurally viable but unstable regions.
          4. Explore — dimension-varying run from the explore_adjacent queue.
          5. Scan    — quality-weighted untested frontier (filtered by avoidance).
        ...
        # ── Step 2: EXPLOIT ───────────────────────────────────────────────────
        exploit_proposal = self._step_exploit_queue(mem)
        if exploit_proposal is not None:
            return exploit_proposal

        # In exploit mode only propose from promising_regions after consuming
        # the explicit exploit queue; otherwise check if the last reflection
        # recommends an exploit run.
        ...
        # ── Step 3: RETEST ────────────────────────────────────────────────────
        retest_proposal = self._step_retest_queue(mem)
        if retest_proposal is not None:
            return retest_proposal
    ...
    def _step_exploit_queue(self, mem: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for entry in exploit_queue:
            ...
            proposal = _scan_support.build_proposal_from_memory_scope(
                self,
                scope,
                description=f"Exploit queued region — {label}",
    ...
    def _step_retest_queue(self, mem: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for entry in retest_queue:
            ...
            proposal = _scan_support.build_proposal_from_memory_scope(
                self,
                scope,
                description=f"Retest structurally viable region — {label}",
                promotion_enabled=False,
